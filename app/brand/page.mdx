...
## {PHILOSOPHY.name} — *{PHILOSOPHY.shona}*
...
### The Five Ubuntu Questions

Every product decision runs through five questions. If the answer to any is no, we do not ship.

<div className="mt-6 grid gap-2">
  {PHILOSOPHY.ubuntuQuestions.map((q, i) => (
...
<div className="mt-6 grid gap-4 sm:grid-cols-3">
  {PHILOSOPHY.triMode.map((mode) => (
    <div key={mode.name} className="rounded-xl border border-border p-4">
      <p className="font-serif text-lg font-semibold text-foreground">{mode.name}</p>
      <p className="font-mono text-xs text-muted-foreground">{mode.meaning}</p>
