...
const STATE_STYLES: Record<CircuitState, { bg: string; text: string; label: string }> = {
  closed: {
    bg: "bg-[var(--color-malachite)]/15",
    text: "text-[var(--color-malachite)]",
    label: "CLOSED",
  },
    ...
    <div className="flex flex-col gap-4 rounded-2xl border border-border bg-card p-4 sm:p-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-muted-foreground">{failures}/3 failures in window</span>
        </div>
      ...
      <div className="flex items-center justify-center gap-2 rounded-xl bg-secondary/30 p-4 text-xs">
        <span
          className={cn(
            ...
            state === "closed"
              ? "bg-[var(--color-malachite)]/20 text-[var(--color-malachite)]"
        ...
        <span
          className={cn(
            "rounded px-2 py-1 font-mono",
            state === "open" ? "bg-destructive/20 text-destructive" : "text-muted-foreground"
          )}
        >
          OPEN
        </span>
      ...
      {/* Log */}
      <div className="max-h-[240px] overflow-y-auto rounded-xl border border-border bg-[#141413] p-3 dark:bg-[#0A0A0A]">
        {log.length === 0 ? (
      ...
      <p className="text-xs text-muted-foreground">
        The circuit opens after 3 failures, rejects all requests for 5 seconds, then enters
        HALF_OPEN to probe. A success closes it; a failure re-opens.
      </p>
    </div>
  )
}
